...
// @match          https://www.markmyprofessor.com/schools*
// @match          https://www.markmyprofessor.com/iskola*
// @match          https://www.markmyprofessor.com/*/iskola*
// @grant          GM.addStyle
...
const UPDATE_INTERVAL = 3 * 7 * 24 * 60 * 60 * 1000;
const CHECK_TIME = 1 * 1000;
const MAX_CHECK_TIME = 10 * 1000;
const CHECK_DELAY = 0.5 * 1000;
...
function NodeFromRow(row, ogName) {
  let returned = document.createElement("div");

  let name = document.createElement("a");
  name.href = row.link;
  name.target = "_blank";
  name.innerText = ogName;

  let score = document.createElement("span");
  score.innerText = `⭐${row.rating}`;
  ...
  returned.addEventListener("click", (event) => {
...
async function GetRow(nameInUrl) {
  return await GM.getValue(`ratings/${nameInUrl}`, null);
}

async function SetRow(nameInUrl, row) {
  GM.setValue(`ratings/${nameInUrl}`, row);
}
...
function IsLecturerString(str) {
  return (
    ...
    str === "Lehrkräfte" ||
    str === "Tárgyfelelős:" ||
    str === "Responsible lecturer:" ||
    str === "Verantwortliche für das Fach:"
  );
}
...
function GetRelevantDivsFromXML(xmlDoc) {
  ...
  try {
    returned = Array.from(
      main.firstChild.lastChild.lastChild.lastChild.childNodes
    );
  } catch (err) {
...
function GetSearchedName() {
  return document.querySelector("input[type=text]:not([name])").value;
}
...
async function GetRelevantRow(rows, name) {
  ...
  for (let dep of await GetDepartments()) {
    let returned = rows.find(
      (row) =>
        row.department === dep &&
        row.rating != "0.00" &&
        row.name.split(" ")[0] === firstName
    );
...
async function SaveName() {
  while (true) {
    let rows = Array.from(GetRowsFromXML(document));
    let name = GetSearchedName();
    ...
    // console.log(name);
    await SetRow(name, relevantRow);
    return;
  }
}

function AddIframe(fullUrl) {
  let frame = document.createElement("iframe");
  frame.src = fullUrl;
  frame.classList.add("mark-my-professor-iframe");
  document.body.appendChild(frame);
  return frame;
}

let beingLookedInto = new Set();

async function UpdateName(name) {
  let fullUrl = GetFullURL(name);
  let urlName = GetNameInURL(name).replaceAll("%20", " ");
  // console.log(urlName);

  let row = await GetRow(urlName);

  if (row !== null && row.updateTime < Date.now() + UPDATE_INTERVAL) {
    ReplaceNames(document, name, row);
    return;
  }

  if (beingLookedInto.has(name)) {
    return;
  }

  beingLookedInto.add(name);

  let frame = AddIframe(fullUrl);

  let id = setInterval(async () => {
    // console.log("ran");

    row = await GetRow(urlName);
...
function GetEffectedTableCells() {
  let returned = [];
...
function GetEffectedElements() {
  let returned = GetEffectedTableCells().concat(GetEffectedSpans());
...
async function UpdateNames() {
  let names = GetNamesFromElements(GetEffectedElements());

  if ((await NoDepartments()) && names.length > 0) {
    if ((await GM.getValue("started", true)) && !IsOnSchoolsPage()) {
      GM.notification({
...
function ReplaceNamesFromCells(cellHolder, name, ratedName) {
  for (let cell of cellHolder.querySelectorAll("td")) {
    ...
    if (
      textEl === null ||
      !textEl.textContent.includes(name) ||
      !cell.hasAttribute(TO_UPDATE_MARK_ATTR) ||
      textEl.nodeType != Node.TEXT_NODE ||
      cell.innerHTML.replace(textEl.textContent, "").includes(name)
    ) {
      continue;
    }
    ...
    cell.appendChild(NodeFromRow(ratedName, name));
...
function ReplaceNamesFromSpans(spanHolder, name, ratedName) {
  for (let span of spanHolder.querySelectorAll("span.tableRowData")) {
    ...
    if (
      textEl === null ||
      !textEl.textContent.includes(name) ||
      !span.hasAttribute(TO_UPDATE_MARK_ATTR) ||
      textEl.nodeType != Node.TEXT_NODE ||
      span.innerHTML.replace(textEl.textContent, "").includes(name)
    ) {
      continue;
    }
    ...
    span.appendChild(NodeFromRow(ratedName, name));
...
function IsOnSchoolPage() {
  return (
    unsafeWindow.location.href.match(
      new RegExp(
        "^https://www.markmyprofessor.com((/.*))?/((iskola)|(school))([^(/)]*)?$"
      )
    ) !== null
  );
}

function IsOnSchoolsPage() {
  return (
    unsafeWindow.location.href.match(
      new RegExp("^https://www.markmyprofessor.com((/.*))?/((iskola)|(school))")
    ) !== null
  );
}
...
function IsOnSearchPage() {
  return (
    unsafeWindow.location.href.match(
      new RegExp("^https://www.markmyprofessor.com/search")
    ) !== null
  );
}
...
function GetSchoolTextNodeForCard(card) {
  ...
  returned.innerText =
    "Click this card to select departments from this school to the Neptun extension";
...
async function GetSwitchForDepartmentCard(card) {
  ...
  await GM.addValueChangeListener(
    "departments",
    async function (key, oldValue, newValue, remote) {
...
function UpdateSchoolTexts() {
  ...
  let cards = document.querySelectorAll(
    "div.cards-grid > div.w-full.rounded.p-4.flex.flex-col.gap-4.bg-white"
  );
...
async function GetDepartmentsList() {
  ...
  returnedBodyText.innerText =
    "Order departments based on relevance. The extension will look up ratings based on this list, prefering departments near the top.";
...
function SetUpStyles() {
  GM.addStyle(
    `
      ...
      .closed.departmentListHolder{
        translate: 0px calc(100% - 4rem);
      }
...
async function SetUpMenus() {
  ...
  const departmentSelectorId = GM.registerMenuCommand(
    "Select departments",
    function (event) {
...
async function OnStart() {
  ...
  GM.setValue("started", !IsOnSchoolsPage());
  if (window.onurlchange === null && IsOnSchoolsPage()) {
